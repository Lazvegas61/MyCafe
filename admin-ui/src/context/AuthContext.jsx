...
export function AuthProvider({ children }) {
        ...
        try {
          ...
          const enhancedUser = {
            ...
            ...(parsedUser.username === 'Admin' && {
              ...
              yetkiler: ['rapor_goruntule', 'gun_baslat', 'gun_kapat', 'kasa_goruntule']
            })
          };
  ...
  const gunSonlandir = () => {
    ...
    return { success: true, message: 'Gün sonlandırıldı' };
  };
  ...
  const hasPermission = (permission) => {
    ...
    const rolePermissions = {
      ADMIN: ['rapor_goruntule', 'gun_baslat', 'gun_kapat', 'kasa_goruntule', 'urun_ekle', 'urun_sil', 'personel_yonet'],
      KASA: ['rapor_goruntule', 'kasa_goruntule', 'gun_kapat'],
      GARSON: ['masa_ac', 'urun_ekle', 'adisyon_olustur'],
      MUTFAK: ['siparis_gor', 'siparis_hazirla']
    };
    
    const permissionsForRole = rolePermissions[user.rol] || [];
  ...
  // Gün başlatma yetkisi kontrolü
  const canStartDay = () => {
    return user?.rol === 'ADMIN' || hasPermission('gun_baslat');
  };

  // Gün sonlandırma yetkisi kontrolü
  const canEndDay = () => {
    return user?.rol === 'ADMIN' || hasPermission('gun_kapat');
  };
